-- Migration to recreate tables without profiles dependency

-- Drop existing tables to ensure a clean slate
DROP TABLE IF EXISTS lobby_players CASCADE;
DROP TABLE IF EXISTS lobbies CASCADE;
DROP TABLE IF EXISTS profiles CASCADE; -- Drop profiles table as it's no longer needed

-- Recreate the lobbies table
CREATE TABLE public.lobbies (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL,
    host_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE, -- Foreign key directly to auth.users
    name TEXT NOT NULL,
...
-- RLS policy for lobbies: Allow authenticated users to insert lobbies, but only if they are the host
...
CREATE TABLE public.lobby_players (
    ...
    player_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE, -- Foreign key directly to auth.users
    ready BOOLEAN NOT NULL DEFAULT FALSE,
    joined_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL,
    PRIMARY KEY (lobby_id, player_id) -- Composite primary key
);
...
FOR DELETE USING (EXISTS (SELECT 1 FROM public.lobbies WHERE id = lobby_id AND host_id = auth.uid()));
